...
class TaskPlannerTest:
    ...
    def create_model(self) -> None:
        ...
        self.decision_variables["delta_ij"] = self.model.addVars(couple_of_tasks_per_agent,
                                                                 ...
                                                                 vtype=gp.GRB.BINARY)
        tasks_per_agent = self.problem_definition.get_tasks_per_agent()
        tasks_couples_per_agent = dict.fromkeys(tasks_per_agent.keys(), [])
        tasks_couples_agent_tuple = []
        for agent in tasks_per_agent:
            for task_couple in itertools.combinations(tasks_per_agent[agent], 2):
                tasks_couples_agent_tuple.append((*task_couple, agent))
        self.decision_variables["V_ijk"] = self.model.addVars(tasks_couples_agent_tuple,
                                                              name="couple_of_tasks_same_agent",
                                                              vtype=gp.GRB.BINARY)

        # print(tasks_couples_agent_tuple)
        # raise Exception
    ...
    def add_general_constraints(self) -> None:
        ...
        #     for task_couple in itertools.combinations(tasks_per_agent[agent], 2):
        #         pass
        for task_i, task_j, agent in self.decision_variables["V_ijk"]:
            ...
            a_i_agent = self.decision_variables["assignment"][(agent, task_i)]
            a_j_agent = self.decision_variables["assignment"][(agent, task_j)]
            self.model.addConstr(t_start_i - t_end_j >= -BIG_M*(2 - a_i_agent - a_j_agent) - BIG_M * (
                        1 - self.decision_variables["V_ijk"][(task_i, task_j, agent)]))
            self.model.addConstr(t_start_j - t_end_i >= -BIG_M*(2 - a_i_agent - a_j_agent) - BIG_M * (
                self.decision_variables["V_ijk"][(task_i, task_j, agent)]))
