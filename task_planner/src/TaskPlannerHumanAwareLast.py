...
@dataclass
class TaskPlannerHumanAwareLast(TaskPlanner):
                    ...
                    if (main_agent, task) in self.decision_variables["assignment"].keys():
                        ...
                        if (task_type, main_agent, parallel_task_type, parallel_agent) in tasks_synergies:
                            ...
                            if self.behaviour == Behaviour.CONTINUOUS:
                                ...
                                # # self.model.addConstr(S_ij >= overlapping * (synergy-1)- BIG_M * (1-assignment_main_task))
                                # # self.model.addConstr(
                                # #     S_ij >= - BIG_M * (assignment_main_task))
                                # # self.model.addConstr(
                                # #     S_ij <=   BIG_M * (assignment_main_task))
                                # #
                                # t_end[task] += S_ij
    ...
    def add_overlapping_constraints(self, upper_bound=None):
        ...
        for task_i, task_j in self.decision_variables["overlapping"].keys():
            ...
            if self.behaviour == Behaviour.DISCRETE:
                ...
                self.model.addConstr((check_parallelism == 1) >> (self.decision_variables["overlapping"][(task_i, task_j)] == (
                        min_t_end - max_t_start)))
                self.model.addConstr(
                    (check_parallelism == 0) >> (self.decision_variables["overlapping"][(task_i, task_j)] == 0))
                ...
                #         min_t_end - max_t_start) * check_parallelism))
                # self.model.addConstr(self.decision_variables["overlapping"][(task_i, task_j)] >= (
                #         min_t_end - max_t_start) - BIG_M * (1- check_parallelism))
                # self.model.addConstr(self.decision_variables["overlapping"][(task_i, task_j)] <= (
                #         min_t_end - max_t_start) - BIG_M * (1- check_parallelism))
                # self.model.addConstr((self.decision_variables["overlapping"][(task_i, task_j)] <=  BIG_M * (check_parallelism)))
                # self.model.addConstr((self.decision_variables["overlapping"][(task_i, task_j)] >=  -BIG_M * (check_parallelism)))
