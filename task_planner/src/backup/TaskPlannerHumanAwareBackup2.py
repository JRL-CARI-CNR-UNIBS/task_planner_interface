...
class TaskPlannerHumanAware(TaskPlanner):
    ...
    def create_model(self) -> None:
        ...
        if self.behaviour == Behaviour.DISCRETE:
            self.decision_variables["overlapping"] = self.model.addVars(copy.copy(tasks_pairs),
                                                                        name="overlapping",
                                                                        vtype=gp.GRB.BINARY,
                                                                        lb=0,
                                                                        ub=1)
            ...
            self.decision_variables["overlapping"] = self.model.addVars(tasks_pairs,
                                                                        ...
                                                                        lb=0,
                                                                        ub=2)
        self.decision_variables["weight"] = self.model.addVars(copy.copy(tasks_pairs),
                                                               name="weight",
                                                               vtype=gp.GRB.CONTINUOUS,
                                                               lb=0,
                                                               ub=1)
        ...
        # print(self.decision_variables["weight"].keys())
        # input("aspetta")
    ...
    def add_t_end_constraints(self, agent_task_combination, cost) -> None:
        ...
        for agent, task in agent_task_combination:
            if task not in t_end:
                ...
                t_end_min[task] = self.decision_variables["t_start"][task]
            t_end[task] += self.decision_variables["assignment"][(agent, task)] * cost[(agent, task)]
            t_end_min[task] += self.decision_variables["assignment"][(agent, task)] * cost[(agent, task)] * 0.7
        ...
        # for task in t_end.keys():
        #     t_end_min[task] = (t_end_min[task]) * 0.8
        #     print(task)
        #     print(t_end_min[task])
        #     input("check")
                ...
                if task in task_pair:
                    ...
                    if (main_agent, task) in self.decision_variables["assignment"].keys():
                        ...
                        if (task_type, main_agent, parallel_task_type, parallel_agent) in tasks_synergies:
                            ...
                            #     overlapping <= min([cost[(main_agent, task)], cost[(parallel_agent, parallel_task)]]))
                            synergy = tasks_synergies[(task_type, main_agent, parallel_task_type, parallel_agent)]
                            print(synergy)
                            delta_ij_estimated = (synergy - 1) * cost[(main_agent, task)]
                            if self.behaviour == Behaviour.CONTINUOUS:
                                t_end[task] += overlapping * delta_ij_estimated * assignment_main_task
                                if synergy > 1:
                                    self.model.addConstr(overlapping >= 1)
                                else:
                                    self.model.addConstr(overlapping <= 1)
                            elif self.behaviour == Behaviour.DISCRETE:  # Add cost (Measurement: time)
                                aux = self.model.addVar(name=f"aux_{task_pair}", vtype=gp.GRB.CONTINUOUS, lb=0, ub=1)
                                print(self.decision_variables["weight"].keys())
                                self.model.addConstr(aux == overlapping * self.decision_variables["weight"][task_pair])
                                t_end[task] += aux * delta_ij_estimated * assignment_main_task
            self.model.addConstr(self.decision_variables["t_end"][task] == t_end[task], name=f"t_end({task})")
            self.model.addConstr(self.decision_variables["t_end"][task] >= t_end_min[task], name=f"t_end_min({task})")
