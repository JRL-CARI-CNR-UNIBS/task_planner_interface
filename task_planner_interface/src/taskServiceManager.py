...
END_MESSAGE = GREEN + "Recipe number: {} successfully performed, cycle time: {}" + END
...
class TaskServiceManager:
    ...
    def __init__(self, agent, agent_type, data, recipe_name, prefix_topic_name, is_human_real, start_recipe_index):
        ...
        # Publisher
        self.pub_feedback_task_planner = rospy.Publisher(prefix_topic_name + agent + "/feedback",
                                                         MotionTaskExecutionFeedback, queue_size=10)
        ...
        self.action_client = actionlib.SimpleActionClient(prefix_topic_name + self.agent + ACTION_NAME,
                                                          TaskExecuteAction)
        self.action_client.wait_for_server(rospy.Duration(5.0))
        self.cancel_pub = rospy.Publisher(prefix_topic_name + self.agent + ACTION_NAME + "/cancel", GoalID,
                                          queue_size=1)

        self.action_client_seq = actionlib.SimpleActionClient(prefix_topic_name + self.agent + ACTION_NAME_SEQ,
                                                              TaskExecuteAction)
    ...
    def execute_client(self, task_name, task_cmd_id, t_start_planned, t_end_planned):  # task_name in realta' è il
        ...
        for agent in agents[agent_type]:
            ...
            if agent_type == "human":
                agent_task_manager.append(
                    TaskServiceManager(agent, agent_type, data, recipe_name, prefix_topic_name, is_human_real,
                                       start_recipe_index))
            else:
                agent_task_manager.append(
